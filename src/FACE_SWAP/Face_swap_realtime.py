...
#init a video writer
result = cv2.VideoWriter(os.path.join(project_path,'output',f'{video_name}.avi'), 
                         cv2.VideoWriter_fourcc(*'MJPG'),
                         10, size)

SOURCE_PATH = os.path.join(project_path,'image',image_name)
#init a dlib face detector
...
while True:
    try:
        ...
        for source_face in source_faces:
            ...
            # hulls = np.reshape(source_face_convexhull,(22,2))
            # for index_hull in np.arange(0,len(hulls)-1,step=1):
            #     cv2.line(source_image,hulls[index_hull],hulls[index_hull+1],(0,255,0),2)
            # cv2.line(source_image,hulls[len(hulls)-1],hulls[0],(0,255,0),2)
            cv2.fillConvexPoly(source_image_canvas, source_face_convexhull, 255)
            # cv2.imshow("Convexhull source face",source_image_canvas)
        ...
        if len(destination_faces) == 1:
            ...
            # cv2.imshow("test",destination_with_face)
            ...
            result.write(seamless_cloned_face)
        else:
            cv2.imshow("final frame", destination_image)
            result.write(destination_image)
